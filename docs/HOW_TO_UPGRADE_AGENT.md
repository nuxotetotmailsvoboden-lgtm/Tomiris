...
4. Publish an owner-approved immutable code release. Phase 03 pilots begin at
   `v0.3-pilot-agents-pass`; never deploy from `main` or a feature branch.
...
Rollout is per Space, so BTC and SOL do not need simultaneous upgrades when ETH changes. Rollback
restores only the ETH Space to its previous compatible certified tag. Phase 02 deployments may
continue running `v0.2-orchestrator-pass`.
